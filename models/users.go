...
type Users struct {
	...
	ApiToken           string    `orm:"column(api_token);null"`
	ApiTokenExpiry     time.Time `orm:"column(api_token_expiry);type(timestamp without time zone)"`
	Language           string    `orm:"column(language);null"`
	Md5                string    `orm:"column(md5);null"`
	Settings           string    `orm:"column(settings);null"`
	Theme              string    `orm:"column(theme);null"`
	Mascot             string    `orm:"column(mascot);null"`
	MascotUrl          string    `orm:"column(mascot_url);null"`
	Pantsu             float64   `orm:"column(pantsu);null"`
	AltColors          string    `orm:"column(alt_colors);null"`
	HideAds            string    `orm:"column(hide_ads);null"`
	OldNav             string    `orm:"column(old_nav);null"`
	AnidexApiToken     string    `orm:"column(anidex_api_token);null"`
	NyaasiApiToken     string    `orm:"column(nyaasi_api_token);null"`
	TokyotoshoApiToken string    `orm:"column(tokyotosho_api_token);null"`
}
